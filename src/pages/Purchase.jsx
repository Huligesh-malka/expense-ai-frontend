    ...
    const loadPurchases = async () => {
        ...
        try {
            const res = await axios.get(
                `${API_BASE_URL}/api/purchases?business_id=${businessId}`,
                {
            ...
            if (err.code === 'ERR_NETWORK') {
                ...
                alert("Failed to load purchases: " + (err.response?.data?.message || err.message));
            }
        } finally {
            setLoading(false);
        }
        ...
        <div style={{
            ...
            {error && (
                <div style={{
                    ...
                    ⚠️ Error: {error}. Please check if backend server is running.
                </div>
            )}
                    ...
                    <tbody>
                        ...
                        {filteredPurchases.map((purchase) => (
                            ...
                            >
                                ...
                                <td>₹{Number(purchase.due_amount).toFixed(2)}</td>
                                <td>
                                    <span style={{
                                        background: purchase.payment_status === "Paid"
                                            ? "#dcfce7"
                                            : purchase.payment_status === "Pending"
                                                ? "#fee2e2"
                                                : "#fef3c7",
                                        color: purchase.payment_status === "Paid"
                                            ? "#15803d"
                                            : purchase.payment_status === "Pending"
                                                ? "#dc2626"
                                                : "#b45309",
